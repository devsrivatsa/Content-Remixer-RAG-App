...
class VectorBaseDocument(ABC, BaseModel, Generic[T]):
    id: UUID4 = Field(default_factory=uuid.UUID4)

    def __eq__(self, value:object) -> bool:
        if not isinstance(value, self._class__):
            return False
    ...
    #this is a recursive function that converts all UUID objects to strings
        ...
        if isinstance(item, dict):
            for k, v in item.items():
                if isinstance(v, UUID):
                    ...
                    item[k] = [self._uuid_to_str(i) for i in v]
                elif isinstance(v, dict):
                    item[k] = {k_:self._uuid_to_str(v_) for k_, v_ in v.items()}
        return item
    ...
    def get_or_create_collection(cls:Type[T]) -> CollectionInfo:
        ...
        try:
            ...
            if collection_created is False:
                raise RuntimeError(f"Could not create collection {collection_name} from None")
    ...
    def group_by_class(cls:Type["VectorBaseDocument"], documents: list["VectorBaseDocument"]) -> Dict["VectorBaseDocument", list["VectorBaseDocument"]]:
        return cls._group_by(documents, selector=lambda doc: doc._class__)
    ...
    #this is just a regular group by operation
    @classmethod
